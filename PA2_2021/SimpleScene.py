...
def getCatmullRom(t, p0, p1, p2, p3):
    res = (t*((2-t)*t-1)*p0
            +(t*t*(3*t-5)+2)*p1
            +t*((4-3*t)*t+1)*p2
            +(t-1)*t*t*p2)/2
...
def getCowDirection(prevPos, nextPos):
    global cowPos
    ...
    Rx = np.identity(3)
    Rx[1:, 1:] = [[np.cos(pitch), -np.sin(pitch)],
                [np.sin(pitch), np.cos(pitch)]]
    Ry = np.array([[np.cos(yaw), 0., np.sin(yaw)],
                    [0.,         1.,          0.],
                    [-np.sin(yaw), 0., np.cos(yaw)]])
    Rz = np.identity(3)
    Rz[:2, :2] = [[np.cos(roll), -np.sin(roll)],
...
def display():
    global cameraIndex, cow2wld, clickCnt, clickLocation, animStartTime, isTimeInit, cowPos
    ...
    if -1 < clickCnt and clickCnt < 6:
        for idx in clickLocation:
            drawCow(idx, True)
    elif clickCnt == 6:
        if not isTimeInit:
            animStartTime = glfw.get_time()
            isTimeInit = True
            cowPos = clickLocation[0].copy()
        
        animTime = glfw.get_time() - animStartTime

        splinePos = initialPos
        t = animTime % 6
        
        if t >= 0 and t < 6 and animTime < 18:
            animTime = float(animTime) - int(animTime)
            if t >= 0 and t < 1:
                splinePos = getCatmullRom(animTime, clickLocation[5], clickLocation[0], clickLocation[1], clickLocation[2])
            elif t >= 1 and t < 2:
                splinePos = getCatmullRom(animTime, clickLocation[0], clickLocation[1], clickLocation[2], clickLocation[3])
            elif t >= 2 and t < 3:
                splinePos = getCatmullRom(animTime, clickLocation[1], clickLocation[2], clickLocation[3], clickLocation[4])
            elif t >= 3 and t < 4:
                splinePos = getCatmullRom(animTime, clickLocation[2], clickLocation[3], clickLocation[4], clickLocation[5])
            elif t >= 4 and t < 5:
                splinePos = getCatmullRom(animTime, clickLocation[3], clickLocation[4], clickLocation[5], clickLocation[0])
            elif t >= 5 and t < 6:
                splinePos = getCatmullRom(animTime, clickLocation[4], clickLocation[5], clickLocation[0], clickLocation[1])
        ...
        getCowDirection(getTranslation(cowPos), getTranslation(splinePos))
        setTranslation(cowPos, getTranslation(splinePos))
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, clickCnt, clickLocation, cow2wld
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if cursorOnCowBoundingBox:
                if clickCnt == -1:
                    clickCnt += 1
                elif -1 < clickCnt and clickCnt < 6:
                    clickCnt += 1
                    clickLocation.append(cow2wld.copy())
                    if clickCnt == 6:
                        isDrag = 0
            else:
                isDrag = 0
