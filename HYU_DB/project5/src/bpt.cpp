...
unordered_map <pair<int,int64_t>,char*, pairHash> record_table;
...
int db_find(int table_id, uint64_t key, char* ret_val, int trx_id) {
    ...
    lock_t* lock_block = lock_acquire(table_id, key, trx_id, SHARED);
    rec = find_record(table_id, root_number, key);
    lock_release(lock_block);
    //lock_t* lock_block = lock_acquire(table_id, key, trx_id, SHARED);
    if (rec == NULL) {
        return -1;
    }
...
int db_update(int table_id, uint64_t key, char* values, int trx_id) {
    record_t* rec;
    buffer_t* buffer_header = get_buffer(table_id, HEADER_NUM);
    page_t* root_page = &buffer_header->frame;
    pagenum_t root_number = ((header_page_t*)root_page)->root_page_number;
    put_buffer(buffer_header);
    lock_t* lock_block = lock_acquire(table_id, key, trx_id, EXCLUSIVE);
    rec = find_record(table_id, root_number, key);
    //lock_t* lock_block = lock_acquire(table_id, key, trx_id, EXCLUSIVE);
    if (rec == nullptr) {
        // Abort
		lock_abort(table_id, root_number, key, trx_id);
		// Release all
		lock_release_all(trx_id);
		// Remove transaction
		lock_remove_trx(trx_id);
        return -1;
    }
    int success = change_record(table_id, root_number, key, values);
    lock_block->is_updated = 1;
    lock_release(lock_block);
    return success;
}

void lock_abort(int table_id, pagenum_t root, uint64_t key, int trx_id) {
    char* org_char;
	trx_t* trx_block = get_trx_block(trx_id);
    lock_t* cur = trx_block->trx_head;
    while (cur != NULL) {
        if (cur->is_updated == 1) {
            auto iter = record_table.find(make_pair(table_id, key));
            org_char = iter->second;
            change_record(table_id, root, key, org_char);
            cur->is_updated = 0;
        }
    }
}
...
int  find_point_index(page_t* page, uint64_t key) {
    int l = 0, r = ((node_page_t*)page)->number_of_keys, m, m_key;
    while (l < r) {
        m = (l + r) / 2;
        if (l == m) return m;
        m_key = find_key(page, m);
        if (m_key <= key) l = m;
        else r = m;
    }
    return -1;
...
int change_record(int table_id, pagenum_t root, uint64_t key, char* ch_val) {
    record_t* rec = NULL;
    page_t* leaf_page;
    pagenum_t c = find_leaf_page(table_id, root, key);
    if (c == HEADER_NUM) {
    ...
    if(((node_page_t*)leaf_page)->is_leaf == 0) {
        return -1;
    }
    strcpy(((node_page_t*)leaf_page)->records[idx].value, ch_val);
    put_buffer(buffer);
...
int db_insert(int table_id, uint64_t key, char* value) {
    ...
    record_table.insert(make_pair(make_pair(table_id, key), value));
...
pagenum_t insert_into_leaf(int table_id, pagenum_t leaf, uint64_t key, char* value) {
    ...
    if (key < find_key(leaf_page, 0)) {
        insertion_point = 0;
    }
    else {
        insertion_point = find_point_index(leaf_page, key);
...
pagenum_t insert_into_leaf_after_splitting(int table_id, pagenum_t root, pagenum_t page_number, uint64_t key, char* value) {
    ...
    if (key < find_key(page, 0)) {
        ...
        insertion_point = find_point_index(page, key) + 1;
    }
...
int db_delete(int table_id, int64_t key) {
    ...
    root_number = delete_(table_id, ((header_page_t*)head)->root_page_number, key);
...
pagenum_t delete_(int table_id, pagenum_t root, uint64_t key) {
...
pagenum_t remove_entry_from_node(int table_id, pagenum_t leaf_number, uint64_t key) {
    ...
    for (j = i; j < ((node_page_t*)leaf)->number_of_keys - 1; j++) {
